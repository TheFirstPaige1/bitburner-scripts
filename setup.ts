import { NS } from "@ns";
import { hasFocusPenalty, quietTheBabblingThrong, masterLister, moneyTimeKill } from "./bitlib";
export async function main(ns: NS): Promise<void> {
	...
	for (const dwprog of dwprogs) {
		if (!ns.fileExists(dwprog, "home")) {
			while (!ns.singularity.purchaseProgram(dwprog)) { await moneyTimeKill(ns, focus); }
		}
	}
	ns.singularity.stopAction();
